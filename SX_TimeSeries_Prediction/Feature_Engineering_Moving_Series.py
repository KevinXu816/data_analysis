...
class moving_series():
    ...
    def __init__(self, dataframe, columns, windows, y_col, alpha=None, beta=None, corr_coef=0.50, min_periods=2,
                 win_type=None):
        if beta is None:
            beta = [0.9]
        if alpha is None:
    ...
    """
    生成与y_col的同时段时间序列相关度，作为特征，
    选取的列需满足与y_col整列的相关度大于corr_coef参数。
    """
...
# dd_MWS = pd.read_csv('/Users/jiaqijiang/Desktop/work/52692_MWS_output.csv', encoding='utf-8').iloc[:, :27]
# pd.set_option('display.max_columns', 100)
# pd.set_option('display.max_rows', 100)
# print(dd_MWS.shape)
# print(dd_MWS.describe)
...
# df = dd_MWS.copy()
#
# ms = moving_series(df, columns_var, windows=[20], y_col='KK1_ABS1_PH1')
